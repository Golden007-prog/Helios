...
export function AnalysisPane({
  ...
  const canQuarantine =
    tier === "confirmed" && Boolean(abend.quarantine_sql);
  ...
  return (
    <div className="flex h-full flex-col gap-3" data-testid="analysis-pane">
      <header>
        <div className="flex items-center justify-between gap-2">
          <h2 className="text-sm font-semibold">
            Identified ABEND:{" "}
            <span className="font-mono">{abend.identified_abend.code}</span>
          </h2>
          <Badge
      ...
      {isUnfamiliar && (
        ...
        >
          <strong className="font-semibold">Hasn't seen this before.</strong>{" "}
          The pattern library couldn't classify this dump. Submit it to the
          learning loop so future runs can match it.
          {onSubmitPattern && (
            <Button
              size="sm"
              className="mt-2 block"
              onClick={onSubmitPattern}
            >
              Submit pattern
            </Button>
          )}
              ...
              >
                <div>
                  ...
                  <div className="text-xs text-fg-muted">
                    {c.prior_count} prior incidents · similarity{" "}
            ...
            {abend.matching_runbooks.map((r) => (
              ...
              >
                <div>
                  <div>{r.title}</div>
                  <div className="text-xs text-fg-muted">
                    used {r.success_count}× successfully
                  </div>
                </div>
                <a
      ...
      <Button
        ...
        onClick={() =>
          canQuarantine && onApplyQuarantine?.(abend.quarantine_sql ?? "")
        }
